...
export const useIo = (namespace?: string) => {
    ...
    // 事件监听
    const on = useCallback((ev: string, listener: (...args: any[]) => void) => {
        if (state.socket) {
    ...
    // 发送事件
    const emit = useCallback((ev: string, ...args: any[]) => {
        if (state.socket) {
    ...
    // 错误处理
    const onError = useCallback((listener: (error: any) => void) => {
        if (state.socket) {
    ...
    // 连接状态变化监听
    const onConnectionChange = useCallback((listener: (isConnected: boolean) => void) => {
        if (state.socket) {
