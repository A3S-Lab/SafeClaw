	...
	return (
		...
		>
			<div className="relative size-10">
				...
				{count > 0 && (
					<Badge className="absolute -top-1 -right-1 flex items-center justify-center p-0 rounded-full size-4 !bg-rose-500 text-white">
						{count}
					</Badge>
				)}
			</div>
			<div className="flex flex-1 flex-col">
				...
				<div className="flex items-center justify-between mt-1">
					<div className="flex-1 text-xs text-muted-foreground/50 max-w-[250px] overflow-hidden whitespace-nowrap text-ellipsis">
						{message}
					</div>
					{isMute && <BellOff className="size-3 text-muted-foreground/50" />}
				</div>
			</div>
		</div>
	);
