            ...
            while let Some(event) = event_rx.recv().await {
                ...
                match &event {
                    ...
                    AgentEvent::ToolEnd {
                        ...
                        let input_str =
                            tool_input_buffers.remove(id).unwrap_or_default();
                            ...
                            if let BrowserIncomingMessage::Result { ref data } = m {
                                ...
                                if let Ok(sessions_guard) = sessions.try_read() {
                                    if let Some(es) = sessions_guard.get(&sid) {
                                        enriched["num_turns"] = serde_json::json!(es.state.num_turns);
                                        enriched["total_cost_usd"] = serde_json::json!(es.state.total_cost_usd);
                                    }
                                }
                                // Add token usage from the End event
                                if let AgentEvent::End { ref usage, .. } = event {
                                    enriched["input_tokens"] = serde_json::json!(usage.prompt_tokens);
                                    enriched["output_tokens"] = serde_json::json!(usage.completion_tokens);
                                    enriched["cache_read_tokens"] = serde_json::json!(usage.cache_read_tokens.unwrap_or(0));
                                    enriched["cache_write_tokens"] = serde_json::json!(usage.cache_write_tokens.unwrap_or(0));
                                    // Estimate context usage: prompt_tokens / max_context (200k default)
                                    let max_ctx = 200_000u64;
                                    let pct = ((usage.prompt_tokens as f64 / max_ctx as f64) * 100.0).min(100.0);
