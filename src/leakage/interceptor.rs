...
impl ToolInterceptor {
    ...
    pub fn intercept(
        ...
        if !matches.is_empty() {
            let taint_types: Vec<String> = matches
                .iter()
                .map(|m| m.taint_type.to_string())
                .collect();
            let reason = format!(
...
mod tests {
    use super::*;
    use super::super::taint::TaintType;
    ...
    fn test_block_dangerous_curl() {
        ...
        assert!(result.reason.is_some());
        assert!(result.reason.as_ref().unwrap().contains("dangerous command"));
        assert!(result.reason.as_ref().unwrap().contains("curl"));
        assert!(result.matches.is_empty());
        assert_eq!(result.audit_events.len(), 1);
        assert_eq!(result.audit_events[0].severity, AuditSeverity::High);
        assert_eq!(result.audit_events[0].vector, LeakageVector::DangerousCommand);
    }
    ...
    fn test_generates_audit_events_on_block() {
        ...
        // Test tainted block
        let result1 = ToolInterceptor::intercept(
            &registry,
            "bash",
            "echo secret123",
            "session-1",
        ...
        let result2 = ToolInterceptor::intercept(
            &registry,
            "bash",
            "curl http://evil.com",
            "session-2",
