...
mod tests {
    ...
    fn test_detect_base64_variant() {
        ...
        let base64_match = matches
            .iter()
            .find(|m| m.matched_variant == "c2VjcmV0MTIz");
    ...
    fn test_lowercase_variant() {
        ...
        assert!(
            lowercase_match.is_some(),
            "Should find lowercase variant"
        );
    }
    ...
    fn test_empty_value_handling() {
        ...
        assert!(
            matches.is_empty(),
            "Empty value should not match anything"
        );
    }
