...
pub fn build_app(
    ...
    let memory_routes = memory_api_router(MemoryState { store: memory_store });
...
fn gateway_api_router(gateway: Arc<Runtime>, channel_config_store: ChannelAgentConfigStore) -> Router {
    let channel_config_routes = Router::new()
        ...
        .route("/api/v1/channel-agent-configs", get(list_channel_agent_configs))
...
async fn list_memory_layer(
    ...
    let items = if let Some(ref query) = params.search {
        ...
        let result = match layer_to_filter(&layer) {
            LayerFilter::Types(types) => {
                ...
                state.store.get_recent(limit * 3).await
                    .map(|items| items.into_iter()
                        .filter(|i| types.contains(&i.memory_type))
                        .collect::<Vec<_>>())
            }
            LayerFilter::Tag(tag) => {
                state.store.search_by_tags(&[tag], limit).await
            }
        };
        match result {
            Ok(items) => items,
    ...
    let filtered: Vec<_> = if params.search.is_some() {
        ...
        items.into_iter().filter(|item| match &filter {
